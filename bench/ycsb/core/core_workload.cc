...
const string CoreWorkload::OPERATIONS_PER_TRANSACTION_PROPERTY =
  "operationspertransaction";
const string CoreWorkload::OPERATIONS_PER_TRANSACTION_DEFAULT = "1";

const string CoreWorkload::HOTKEY_PERCENTAGE_PROPERTY = "hotkeypercentage";
const string CoreWorkload::HOTKEY_PERCENTAGE_DEFAULT = "0";

const string CoreWorkload::COUNTER_INITIAL_PROPERTY = "countervalue";
const string CoreWorkload::COUNTER_INITIAL_DEFAULT = "1";
...
CoreWorkload::Init(const utils::Properties& p)
{
  ...
  ops_per_txn_ = std::stoi(p.GetProperty(OPERATIONS_PER_TRANSACTION_PROPERTY,
                                         OPERATIONS_PER_TRANSACTION_DEFAULT));

  hot_key_perc_ = std::stoi(
    p.GetProperty(HOTKEY_PERCENTAGE_PROPERTY, HOTKEY_PERCENTAGE_DEFAULT));
  counter_init_val_ =
    std::stoi(p.GetProperty(COUNTER_INITIAL_PROPERTY, COUNTER_INITIAL_DEFAULT));
  ...
  if (request_dist == "uniform") {
    ...
    key_chooser_->Seed(std::stoi(p.GetProperty("globalid")));
    ...
    int op_count =
      std::stoi(p.GetProperty(OPERATION_COUNT_PROPERTY)) + ops_per_txn_;
...
void
CoreWorkload::BuildValue(std::string& value)
{
